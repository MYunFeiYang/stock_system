...
def run_walkforward(replay_days: int | None = None, fetch_days: int = 300,
                   gated: bool = False, dump_raw: bool = False) -> dict:
    ...
    for stock in stocks:
        ...
        for i in range(start, end + 1):
            ...
            rows.append({
                ...
                # ── 价量特征（供离线均值回归/因子实验重筛，不参与原信号决策）──
                "rsi": round(float(technical.get("rsi", 50.0)), 2),
                "momentum_5d": round(float(technical.get("momentum_5d", 0.0)), 4),
                "momentum_10d": round(float(technical.get("momentum_10d", 0.0)), 4),
                "momentum_20d": round(float(technical.get("momentum_20d", 0.0)), 4),
                "ma_alignment": technical.get("ma_alignment", "中性"),
                "ma20": technical.get("ma20"),
                "ma60": technical.get("ma60"),
                "bollinger_position": round(float(technical.get("bollinger_position", 0.5)), 3),
                "macd_signal": technical.get("macd_signal", "中性"),
                "volume_ratio": round(float(technical.get("volume_ratio", 1.0)), 3),
    ...
    if dump_raw:
        rdays = sorted({r["pred_day"] for r in rows})
        raw_path = "data/walkforward_raw.json"
        with open(raw_path, "w", encoding="utf-8") as rf:
            json.dump({
                "generated_at": datetime.datetime.now().isoformat(timespec="seconds"),
                "range": {"start": rdays[0], "end": rdays[-1],
                          "trading_days": len(rdays)},
                "gated": gated,
                "rows": rows,
            }, rf, ensure_ascii=False, indent=2)
        print(f"\n💾 raw 逐笔已落盘: {raw_path}（{len(rows)} 条，含价量特征）")
...
def main():
    ...
    ap.add_argument("--dump-raw", action="store_true",
                    help="落盘 raw 逐笔（含价量特征）到 data/walkforward_raw.json，"
                         "供离线均值回归/因子实验重筛")
    args = ap.parse_args()
    run_walkforward(replay_days=args.days, fetch_days=args.fetch,
                    gated=args.gated, dump_raw=args.dump_raw)
